...
def getNatVisitor(yyyymm, nat_cd, ed_cd):
    end_point = "http://openapi.tour.go.kr/openapi/service/EdrontToursimStateService/getEdrcntTourismStatsList"
    ...
    for year in range(nStartYear, nEndYear):
        for month in range(1, 13):
            ...
            if jsonData['response']['header']['resultMsg'] == 'OK':
                ...
                iTotalVisite = jsonData['response']['body']['items']['item']['num']
                print('%s_%s:%s' % (krName, yyyymm, iTotalVisite))

                jsonResult.append(
                    {'nat_name': krName, 'nat_cd': national_code, 'yyyymm': yyyymm, 'visit_cnt': iTotalVisite})
